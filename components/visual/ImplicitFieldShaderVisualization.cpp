...
ImplicitFieldShaderVisualization::ImplicitFieldShaderVisualization() :
    ...
    d_viewportCoordinate(initData(&d_viewportCoordinate, {-1.0,-1.0,1.0,1.0}, "viewportCoordinates", "The viewport coordinates where the shader will draw the scene")),
...
void ImplicitFieldShaderVisualization::shaderGenerationCodeHasChanged()
{
    ...
    std::string newUniforms = uniformsAndConst(glslMap);
    std::string newImplicit = implicitFunction(glslMap);
    if( newUniforms != m_prevUniforms || newImplicit != m_prevCode )
    {
        msg_info() << "(Re)Generating the shader for the rendering of the implicit shape";
        initComponentShaderValue();
        std::ofstream myfile;
        myfile.open (d_fragFilename.getFullPath());
        generateFragmentShader(myfile, newUniforms, newImplicit);
        myfile.close();

        myfile.open (d_vertFilename.getFullPath());
        generateVertexShader(myfile, newUniforms, newImplicit);
        myfile.close();

        m_prevUniforms = newUniforms;
        m_prevCode = newImplicit;
    }
}
...
{
    if( vp->pass() == VisualParams::Transparent )
        return;
    ...
    auto viewportCoordinates = d_viewportCoordinate.getValue();
    ...
    glTexCoord2d(viewportCoordinates[0],viewportCoordinates[1]);
    glVertex2d(  viewportCoordinates[0],viewportCoordinates[1]);

    glTexCoord2d(viewportCoordinates[0],viewportCoordinates[3]);
    glVertex2d(  viewportCoordinates[0],viewportCoordinates[3]);

    glTexCoord2d(viewportCoordinates[2], viewportCoordinates[3]);
    glVertex2d(  viewportCoordinates[2], viewportCoordinates[3]);

    glTexCoord2d(viewportCoordinates[2],viewportCoordinates[1]);
    glVertex2d(  viewportCoordinates[2],viewportCoordinates[1]);
    glEnd();
...
std::ostream& ImplicitFieldShaderVisualization::generateVertexShader(std::ostream& out, const std::string &uniforms, const std::string &implicit)
{
...
std::string ImplicitFieldShaderVisualization::uniformsAndConst(std::map<std::string, std::vector<GLSLCodeFragment>> glslMap)
{
    std::string tmp;
    ...
    if(itFind != glslMap.end())
    {
        ...
        {
            GLSLCodeFragment tmpGLSLCode = *it;
            tmp.append("uniform " + tmpGLSLCode.m_type + " " + tmpGLSLCode.m_dataname + "; ///ICI \n");
        }
    }
    tmp.append("\n");
...
std::string ImplicitFieldShaderVisualization::implicitFunction(std::map<std::string, std::vector<GLSLCodeFragment>> glslMap)
{    
...
std::ostream& ImplicitFieldShaderVisualization::generateFragmentShader(std::ostream& out, const std::string& uniforms, const std::string& implicit)
{
    std::ostringstream istr ;
    std::ifstream input;
    ...
    tmp = std::regex_replace (istr.str(), r1, uniforms);

    std::regex r2 ("REPLACE_WITH_SHAPE");
    tmp = std::regex_replace(tmp, r2, implicit);
...
static int IC = sofa::core::RegisterObject("Render an implicit field using a ray marching shader")
        .add<ImplicitFieldShaderVisualization>();
